...
The sequence model uses attention pooling over all hidden states generated by the GRU. Instead of relying only on the last window, the classifier aggregates information from the entire sequence and assigns higher weight to the most relevant parts. Labels are assigned per sequence (positive if any window contains a seizure).
